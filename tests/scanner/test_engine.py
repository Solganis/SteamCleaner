...
class TestCustomPaths:
    def test_scans_custom_directory(self, tmp_path: Path):
        ...
        assert any(entry.client_name == "Custom" for entry in result.entries)
        assert any("setup.exe" in str(entry.path) for entry in result.entries)
    ...
    def test_custom_path_progress_callback(self, tmp_path: Path):
        ...
        assert any("MyLibrary" in msg for msg in messages)
